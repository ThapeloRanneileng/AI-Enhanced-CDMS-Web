...
def _post_json(url: str, headers: Dict[str, str], payload: Dict[str, Any], timeout: int = 30) -> Dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError("GenAI provider request failed") from exc
...
class GroqProvider(ExternalGenAIProvider):
    ...
    def __init__(self, api_key: str, model: str, fallback: GenAIProvider | None = None) -> None:
        self.api_key = api_key
        self.model = model
...
def select_provider(mode: str | None = None) -> GenAIProvider:
    ...
    if selected == "gemini":
        ...
        return fallback
    if selected == "groq":
        return select_groq_or_template()
